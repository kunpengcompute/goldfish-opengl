...
#ifndef __GRALLOC_CB_BP_H__
#define __GRALLOC_CB_BP_H__

#include <cutils/native_handle.h>
#include "../../shared/qemupipe/include-types/qemu_pipe_types_bp.h"
...
struct cb_handle_t_v11 : public native_handle_t {
    cb_handle_t_v11(int32_t p_bufferFd,
    ...
    static cb_handle_t_v11* from(void* p) {
        if (!p) { return NULL; }
        cb_handle_t_v11* cb = static_cast<cb_handle_t_v11*>(p);
        return cb->isValid() ? cb : NULL;
    }

    static const cb_handle_t_v11* from(const void* p) {
        return from(const_cast<void*>(p));
    }

    static cb_handle_t_v11* from_unconst(const void* p) {
        return from(const_cast<void*>(p));
    }
