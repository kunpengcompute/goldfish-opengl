 ...
 * 执行 Vulkan 本地渲染状态机
*/
#define EXEC_VULKAN_STATE_MACHINE(RET, FUNC, ARGS...)                                   \
do {                                                                                    \
    RET = VK_SUCCESS;                                                                   \
    static void * lvpHandle = dlopen("/vendor/lib64/libvulkan_lvp.so", RTLD_NOW);       \
    if (lvpHandle == NULL) {                                                            \
        ALOGE("Open libvulkan_lvp.so failed, %s", dlerror());                           \
        RET = VK_ERROR_DEVICE_LOST;                                                     \
        break;                                                                          \
    }                                                                                   \
    static std::string funcName = std::string("lvp_") + std::string(#FUNC);             \
    static PFN_vk##FUNC func = (PFN_vk##FUNC)dlsym(lvpHandle, funcName.c_str());        \
    if (func == NULL) {                                                                 \
        ALOGE("Get function %s failed, %s", funcName.c_str(), dlerror());               \
        RET = VK_ERROR_DEVICE_LOST;                                                     \
        break;                                                                          \
    }                                                                                   \
    RET = func(ARGS);                                                                   \
...
#define EXEC_VULKAN_STATE_MACHINE_VOID(RET, FUNC, ARGS...)                              \
do {                                                                                    \
    RET = VK_SUCCESS;                                                                   \
    static void * lvpHandle = dlopen("/vendor/lib64/libvulkan_lvp.so", RTLD_NOW);       \
    if (lvpHandle == NULL) {                                                            \
