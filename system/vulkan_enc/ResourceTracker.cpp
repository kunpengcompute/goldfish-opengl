...
#include "shared/GoldfishAddressSpace/include/goldfish_address_space.h"
#include "goldfish_vk_private_defs.h"
...
#define RESOURCE_TRACKER_DEBUG 1
...
#define HANDLE_UNREGISTER_IMPL_IMPL(type) \
    ...
    VkDeviceSize getAllocationSize(VkDeviceMemory memory) {
        AutoLock lock(mLock);
        const auto it = info_VkDeviceMemory.find(memory);
    ...
    VkResult on_vkEnumerateInstanceExtensionProperties(
        ...
        if (mHostInstanceExtensions.empty()) {
            // 获取 Vulkan 本地状态机的扩展属性列表，其能力当前与 Kbox9 一致。
            VkResult hostRes;
            uint32_t hostPropCount = 0;
            EXEC_VULKAN_STATE_MACHINE(hostRes, EnumerateInstanceExtensionProperties, nullptr, &hostPropCount, nullptr);
            mHostInstanceExtensions.resize(hostPropCount);

            EXEC_VULKAN_STATE_MACHINE(hostRes, EnumerateInstanceExtensionProperties,
                nullptr, &hostPropCount, mHostInstanceExtensions.data());
            if (hostRes != VK_SUCCESS) {
                ALOGE("Call vulkan state machine vkEnumerateInstanceExtensionProperties failed. ret:%#x", hostRes);
                return hostRes;
            }
        }
...
#ifdef VK_USE_PLATFORM_ANDROID_KHR
            filteredExts.push_back(VkExtensionProperties {
    ...
    VkResult on_vkCreateInstance(
        ...
        EXEC_VULKAN_STATE_MACHINE(enumInstanceVersionRes, EnumerateInstanceVersion, &apiVersion);
    ...
    VkResult on_vkAllocateMemory(
        ...
        if (!directMappingSupported) {
            ...
            D("host visible alloc (non-direct): "
              "host ptr %p, pMemory %jd, allocation Size 0x%llx, mapped size 0x%llx",
              mappedPtr, *pMemory,
...
VkDeviceSize ResourceTracker::getAllocationSize(VkDeviceMemory memory) {
    return mImpl->getAllocationSize(memory);
}
