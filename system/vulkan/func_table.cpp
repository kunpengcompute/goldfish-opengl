...
// 针对Host和Device共享的映射内存，提供额外的同步机制，保证Host和Device的内存一致
// TODO：根据内存属性，可以判断出部分内存是从Host向Device同步，部分内存是Device向Host同步，从而减少同步量
// TODO：可以计算内存的差异部分，只传输差异部分而不是整块内存，减少传输量
// TODO：可以将多块内存合并用一条命令传输
class MapMemorySync {
public:
    static MapMemorySync& GetInstance()
    {
        static MapMemorySync mapMemorySync;
        return mapMemorySync;
    }

    void MapMemory(VkDevice device, VkDeviceMemory memory, VkDeviceSize offset, VkDeviceSize size, VkMemoryMapFlags flags)
    {
        std::lock_guard<std::recursive_mutex> lk(m_mapMemoryMutex);
        MemoryMapInfo& info = m_mapMemoryRecord[device][memory];
        info.device = device;
        info.memory = memory;
        info.offset = offset;
        info.size = size;
        info.flags = flags;
        ALOGD("MapMemorySync - MapMemory device:%jd, memory:%jd, offset:%jd, size:%jd, flags:%#x",
    ...
    {
        std::lock_guard<std::recursive_mutex> lk(m_mapMemoryMutex);
        m_mapMemoryRecord[device].erase(memory);
        ALOGD("MapMemorySync - UnmapMemory device:%jd, memory:%jd", device, memory);
    }

    // 将一块Host内存向Device内存同步
    void FlushOne(VkDevice device, VkDeviceMemory memory)
    {
        std::lock_guard<std::recursive_mutex> lk(m_mapMemoryMutex);
        if (IsMemoryMap(device, memory) == false) {
            ALOGE("MapMemorySync - Flush one memory but not exist. device:%jd, memory:%jd", device, memory);
            return;
        }

        MemoryMapInfo& info = m_mapMemoryRecord[device][memory];
        ALOGD("MapMemorySync - FlushOne device:%jd, memory:%jd, offset:%jd, size:%jd",
            device, memory, info.offset, info.size);

        VkMappedMemoryRange mappedRange;
        mappedRange.sType = VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE;
        mappedRange.pNext = nullptr;
        mappedRange.memory = memory;
        mappedRange.offset = info.offset;
        mappedRange.size = info.size;
        entry_vkFlushMappedMemoryRanges(device, 1, &mappedRange);
    }

    // 将所有Host内存向Device内存同步
    void FlushAll()
    {
        std::lock_guard<std::recursive_mutex> lk(m_mapMemoryMutex);
        ALOGD("MapMemorySync - FlushAll");
        for(auto& itDevice : m_mapMemoryRecord) {
            for(auto& itMemory : itDevice.second) {
                FlushOne(itMemory.second.device, itMemory.second.memory);
            }
        }
    }

    // 将一块Device内存向Host内存同步
    void InvalidateOne(VkDevice device, VkDeviceMemory memory)
    {
        std::lock_guard<std::recursive_mutex> lk(m_mapMemoryMutex);
        if (IsMemoryMap(device, memory) == false) {
            ALOGE("MapMemorySync - Invalidate one memory but not exist. device:%jd, memory:%jd", device, memory);
            return;
        }

        MemoryMapInfo& info = m_mapMemoryRecord[device][memory];
        ALOGD("MapMemorySync - InvalidateOne device:%jd, memory:%jd, offset:%jd, size:%jd",
            device, memory, info.offset, info.size);
        ...
        mappedRange.offset = info.offset;
        mappedRange.size = info.size;
        entry_vkInvalidateMappedMemoryRanges(device, 1, &mappedRange);
    }

    // 将所有Device内存向Host内存同步
    void InvalidateAll()
    {
        std::lock_guard<std::recursive_mutex> lk(m_mapMemoryMutex);
        ALOGD("MapMemorySync - InvalidateAll");
        for(auto& itDevice : m_mapMemoryRecord) {
            for(auto& itMemory : itDevice.second) {
                InvalidateOne(itMemory.second.device, itMemory.second.memory);
            }
        }
    }
    ...
    {
        std::lock_guard<std::recursive_mutex> lk(m_mapMemoryMutex);
        ALOGD("MapMemorySync - ShowAll");
        int index = 1;
        for(auto& itDevice : m_mapMemoryRecord) {
            for(auto& itMemory : itDevice.second) {
                ALOGD("\tMapMemorySync - list(%04d): device:%jd, memory:%jd",
                    index++, itMemory.second.device, itMemory.second.memory);
            }
        }
    }

private:
    bool IsMemoryMap(VkDevice device, VkDeviceMemory memory)
    {
        auto itDevice = m_mapMemoryRecord.find(device);
        if (itDevice == m_mapMemoryRecord.end()) {
            return false;
        }

        auto& mapMemory = itDevice->second;
        if (mapMemory.find(memory) == mapMemory.end()) {
            return false;
        }
    ...
    std::map<VkDevice, std::map<VkDeviceMemory, MemoryMapInfo>> m_mapMemoryRecord;
    std::recursive_mutex m_mapMemoryMutex;
};
...
{
    AEMU_SCOPED_TRACE("vkCreateInstance with local state machine");
...
static void entry_vkDestroyInstance(
    ...
    const VkAllocationCallbacks* pAllocator)
{
    AEMU_SCOPED_TRACE("vkDestroyInstance with local state machine");
    auto vkEnc = HostConnection::get()->vkEncoder();
...
{
    AEMU_SCOPED_TRACE("vkEnumeratePhysicalDevices with local state machine");
    auto vkEnc = HostConnection::get()->vkEncoder();
...
{
    AEMU_SCOPED_TRACE("vkEnumerateInstanceExtensionProperties with local state machine");
    auto vkEnc = HostConnection::get()->vkEncoder();
    ...
    VkFence fence)
{
    ...
    // 在图形指令提交给执行队列之前，将主机内存的内容同步到显存
    MapMemorySync::GetInstance().FlushAll();
    ...
    void** ppData)
{
    ...
    if (vkMapMemory_VkResult_return == VK_SUCCESS) {
        MapMemorySync::GetInstance().MapMemory(device, memory, offset, size, flags);
    }
    ...
    VkDeviceMemory memory)
{
    ...
    MapMemorySync::GetInstance().FlushOne(device, memory);
    MapMemorySync::GetInstance().UnmapMemory(device, memory);
    ...
    const VkMappedMemoryRange* pMemoryRanges)
{
    ...
    // 受限 SIPC 最大传 8MB 的内容，此处将每一块映射内存分开传输
    for (uint32_t count = 0; count < memoryRangeCount; count++) {
        ALOGD("Sendind memory(%jd), offset(%jd), size(%jd)", pMemoryRanges[count].memory,
            pMemoryRanges[count].offset, pMemoryRanges[count].size);

        VkMappedMemoryRange mappedRange;
        mappedRange.sType = pMemoryRanges[count].sType;
        mappedRange.pNext = nullptr;
        mappedRange.memory = pMemoryRanges[count].memory;

        VkDeviceSize memorySize = pMemoryRanges[count].size;
        if (pMemoryRanges[count].size == VK_WHOLE_SIZE) {
            memorySize = resources->getAllocationSize(pMemoryRanges[count].memory);
        }
        VkDeviceSize flushSize = memorySize - pMemoryRanges[count].offset;

        // 受限 SIPC 最大传 8MB 的内容，此处针对较大的 memory 切分成 4MB 大小的块分次更新
        const VkDeviceSize maxSendSize = 4 * 1024 * 1024;
        if (flushSize <= maxSendSize) {
            mappedRange.offset = pMemoryRanges[count].offset;
            mappedRange.size = pMemoryRanges[count].size;
            vkFlushMappedMemoryRanges_VkResult_return = vkEnc->vkFlushMappedMemoryRanges(device, 1, &mappedRange);
        } else {
            VkDeviceSize offset = pMemoryRanges[count].offset;
            VkDeviceSize leftSize = flushSize;

            while (leftSize > 0) {
                mappedRange.offset = offset;
                VkDeviceSize sendSize = leftSize > maxSendSize ? maxSendSize : leftSize;
                mappedRange.size = sendSize;

                ALOGD("Sendind memory(%jd) block, offset(%jd), size(%jd)",
                    pMemoryRanges[count].memory, offset, sendSize);

                vkFlushMappedMemoryRanges_VkResult_return = vkEnc->vkFlushMappedMemoryRanges(device, 1, &mappedRange);
                if (vkFlushMappedMemoryRanges_VkResult_return != 0) {
                    break;
                }
...
{
    AEMU_SCOPED_TRACE("vkEnumerateInstanceVersion with local state machine");
    VkResult ret;
    EXEC_VULKAN_STATE_MACHINE(ret, EnumerateInstanceVersion, pApiVersion);
    if (ret != VK_SUCCESS) {
        ALOGE("Call vulkan state machine vkEnumerateInstanceVersion failed. ret:%#x", ret);
        *pApiVersion = 0;
        return ret;
    }
    ALOGI("Vulkan Instance Version:%#x", *pApiVersion);
    return ret;
}
...
void* goldfish_vulkan_get_proc_address_inner(const char* name){
#ifdef VK_VERSION_1_0
...
void* goldfish_vulkan_get_proc_address(const char* name){
    static int i = 1;
    void* func = goldfish_vulkan_get_proc_address_inner(name);
    ALOGD("Get function %s, return %p, count:%d", name, func, i++);
    return func;
}

void* goldfish_vulkan_get_instance_proc_address_inner(VkInstance instance, const char* name){
    auto resources = ResourceTracker::get();
...
void* goldfish_vulkan_get_instance_proc_address(VkInstance instance, const char* name){
    static int i = 1;
    void* func = goldfish_vulkan_get_instance_proc_address_inner(instance, name);
    ALOGD("Get instance %#lu function %s, return %p, count:%d", (uint64_t)instance, name, func, i++);
    return func;
}

void* goldfish_vulkan_get_device_proc_address_inner(VkDevice device, const char* name){
    auto resources = ResourceTracker::get();
...
void* goldfish_vulkan_get_device_proc_address(VkDevice device, const char* name){
    static int i = 1;
    void* func = goldfish_vulkan_get_device_proc_address_inner(device, name);
    ALOGD("Get device %#lu function %s, return %p, count:%d", (uint64_t)device, name, func, i++);
    return func;
}
